...
// IMPORTANT: must match your npm package name (and your GitHub repo package.json "name")
const PLUGIN_NAME = 'homebridge-bravia-favourites-hdmi';
const PLATFORM_NAME = 'BraviaFavourites';
...
class BraviaFavouritesPlatform {
  constructor(log, config, api) {
    ...
    // Homebridge will call constructor even when it is just loading plugin metadata sometimes
    if (!api || !config) return;

    if (!this.psk) {
      this.log(`[${PLATFORM_NAME}] Missing required config: psk`);
      return;
    }
    if (!this.tvs.length) {
      this.log(`[${PLATFORM_NAME}] No TVs configured.`);
      return;
    }
    ...
    this.api.on('didFinishLaunching', () => {
      ...
      for (const tvCfg of this.tvs) {
        ...
        const uuid = UUIDGen.generate(`${tvCfg.name}-${PLATFORM_NAME}-${tvCfg.ip}`);
        ...
        if (cachedAccessory) {
          ...
          // Not strictly required, but harmless if context changed
          ...
          this.log(`[${PLATFORM_NAME}] Registered new accessory: ${tvCfg.name}`);
        }
      }
      ...
      if (stale.length) {
        this.log(`[${PLATFORM_NAME}] Removing ${stale.length} stale accessory(ies)`);
...
`;
        ...
        this.log(`[${PLATFORM_NAME}] Created starter favourites file at ${this.favouritesFile}`);
      }
    } catch (e) {
      this.log(`[${PLATFORM_NAME}] Failed to ensure favourites file: ${e && e.message ? e.message : e}`);
    }
  }
  ...
  loadFavourites() {
    ...
    try {
      ...
      this.log(`[${PLATFORM_NAME}] Failed to load favourites: ${e && e.message ? e.message : e}`);
    }
...
class BraviaTvController {
  ...
  buildServices() {
    ...
    this.tvService.getCharacteristic(Characteristic.Active)
      ...
      .on('set', (value, cb) => {
        this.setPower(!!value)
          .then(() => cb(null))
          .catch(err => {
            this.log(`[${PLATFORM_NAME}] ${this.name} setPower error: ${err && err.message ? err.message : err}`);
            cb(null);
          });
      });
    ...
    this.tvService.getCharacteristic(Characteristic.ActiveIdentifier)
      ...
      .on('set', (identifier, cb) => {
        this.selectIdentifier(identifier)
          .then(() => cb(null))
          .catch(err => {
            this.log(`[${PLATFORM_NAME}] ${this.name} selectIdentifier error: ${err && err.message ? err.message : err}`);
            cb(null);
          });
      });
    ...
    if (this.debug) {
      this.log(
        `[${PLATFORM_NAME}] ${this.name} inputs: favourites=${this.identifierToChannel.size}, hdmi=${this.identifierToHdmiPort.size}`
      );
    }
  }
  ...
  startPollingPower() {
    const poll = async () => {
      try {
        ...
        if (this.debug) this.log(`[${PLATFORM_NAME}] ${this.name} power poll error: ${e && e.message ? e.message : e}`);
      } finally {
  ...
  async setPower(on) {
    if (this.debug) this.log(`[${PLATFORM_NAME}] ${this.name} setPower(${on})`);
  ...
  async selectIdentifier(identifier) {
    ...
    if (hdmiPort) {
      if (this.debug) this.log(`[${PLATFORM_NAME}] ${this.name} switching to HDMI ${hdmiPort} (identifier ${identifier})`);
    ...
    if (this.debug) this.log(`[${PLATFORM_NAME}] ${this.name} tuning channel ${channel} (identifier ${identifier})`);
  ...
  async refreshChannelMap() {
    ...
    if (!Array.isArray(list) || !list.length) {
      if (this.debug) this.log(`[${PLATFORM_NAME}] ${this.name} getContentList returned no entries for source=${this.tvsource}`);
      return;
    }
    ...
    if (this.debug) {
      this.log(`[${PLATFORM_NAME}] ${this.name} channel URI map loaded: ${newMap.size} entries`);
    }
  }
  ...
  async refreshHdmiMap() {
    ...
    const list = (resp && resp.result && resp.result[0]) ? resp.result[0] : [];
    if (!Array.isArray(list) || !list.length) {
      if (this.debug) this.log(`[${PLATFORM_NAME}] ${this.name} getCurrentExternalInputsStatus returned no entries`);
      return;
    }
    ...
    if (this.debug) {
      this.log(`[${PLATFORM_NAME}] ${this.name} HDMI URI map loaded: ${newMap.size} entries`);
    }
  }
  ...
  async tuneChannel(channelNumber) {
    ...
    if (!channelUri) {
      this.log(`[${PLATFORM_NAME}] ${this.name} no channel URI for ${ch}. Check tvsource and Live TV availability.`);
      return;
    }

    if (this.debug) this.log(`[${PLATFORM_NAME}] ${this.name} setPlayContent uri=${channelUri}`);
  ...
  async switchToHdmi(hdmiPort) {
    ...
    if (!uri) {
      this.log(`[${PLATFORM_NAME}] ${this.name} no HDMI URI found for HDMI ${portStr}. TV API may not expose it, or port parsing failed.`);
      return;
    }

    if (this.debug) this.log(`[${PLATFORM_NAME}] ${this.name} setPlayContent (HDMI ${portStr}) uri=${uri}`);
